import {
  ...
  OutputMessage,
  RunCodeOptions
} from '../types/code-interpreter';
import { DEFAULT_TIMEOUT_MS, METALLIC_JUPYTER_PORT } from '../utils/constants';
import { MetallicError } from '../utils/error';
import { Execution, Result } from '../utils/jupyter';
...
export class CodeInterpreter {
  ...
  protected get jupyterUrl(): string {
    return `https://${this.templateSlug}-${this.instanceId}-${METALLIC_JUPYTER_PORT}.metallic.computer`;
  }
  ...
  async run(
    ...
    try {
      ...
      if (!res.body) {
        throw new MetallicError(`No response body: ${res.statusText} ${await res?.text()}`);
      }
      ...
      try {
        for await (const chunk of CodeInterpreter.readLines(res.body)) {
          await CodeInterpreter.parseOutput(
            execution,
            chunk,
            options?.onStdout,
            options?.onStderr,
        ...
        throw CodeInterpreter.formatExecutionTimeoutError(err);
      } finally {
      ...
      throw CodeInterpreter.formatRequestTimeoutError(err);
    }
  }
  ...
  private static formatExecutionTimeoutError(err: unknown) {
    if (err instanceof Error && err.name === 'AbortError') {
      return new MetallicError("Execution timed out — the 'timeoutMs' option can be used to increase this timeout");
    }
    return err;
  }

  private static formatRequestTimeoutError(err: unknown) {
    if (err instanceof Error && err.name === 'AbortError') {
      return new MetallicError(
